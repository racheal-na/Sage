...
exports.getConstitution = catchAsync(async (req, res) => {
  if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
    return res.status(400).json({ success: false, message: 'Invalid constitution ID' });
  }
...
exports.createConstitution = catchAsync(async (req, res) => {
  ...
  if (!title || !fileUrl || !fileName) {
    return res.status(400).json({ success: false, message: 'Title, fileUrl, and fileName are required.' });
  }
